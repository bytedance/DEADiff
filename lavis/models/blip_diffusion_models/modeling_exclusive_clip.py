...
class CtxCLIPEmbedder(nn.Module):
...
class CtxCLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        if ctx_embeddings is not None:
            ...
            last_hidden_state = torch.cat([last_hidden_state, torch.zeros(bsz, input_shape[1] - seq_len, last_hidden_state.shape[-1]).to(last_hidden_state.device)], dim=1)
        else:
            pooled_output = last_hidden_state[
                torch.arange(last_hidden_state.shape[0], device=input_ids.device),
...
class CtxCLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if ctx_embeddings is None:
            ...
            seq_length = ctx_embeddings.shape[1]
        ...
        if inputs_embeds is None:
            ...
            if ctx_embeddings is not None:
                inputs_embeds = ctx_embeddings
        # print('here: ', inputs_embeds.shape, position_ids.shape, input_ids.shape, ctx_begin_pos)
